   ...
   var serverQueue = message.client.queue.get(message.guild.id);
    ...
    if (url.match(/^(https?:\/\/)?(www\.)?(m\.)?(youtube\.com|youtu\.?be)\/.+$/gi)) {
       try {
          ...
          if(!songInfo)return sendError("YouTubeで曲が見つからなかったようです", message.channel);
        song = {
    ...
    if(searched.videos.length === 0)return sendError("YouTubeで曲が見つからなかったようです", message.channel)
...
stream.on('error', function(er)  {
    ...
    try {
      ...
      console.error(`ボイスチャンネルに参加できませんでした: ${error}`);
      message.client.queue.delete(message.guild.id);
      await channel.leave();
      return sendError(`ボイスチャンネルに参加できませんでした: ${error}`, message.channel);
    }
