...
module.exports = {
  ...
  run: async function (client, message, args) {
   const queue = message.client.queue.get(message.guild.id);
    if (!queue) return sendError("キューはありません",message.channel).catch(console.error);
    ...
    if (queue.songs.length == 1) return sendError("キューはありません。",message.channel).catch(console.error);
    if (args[0] > queue.songs.length)
      return sendError(`キューの長さはたったの${queue.songs.length}曲です！`,message.channel).catch(console.error);
try{
    const song = queue.songs.splice(args[0] - 1, 1); 
    sendError(`❌ **|** Removed: **\`${song[0].title}\`** from the queue.`,queue.textChannel).catch(console.error);
                   message.react("✅")
} catch (error) {
        return sendError(`:notes: 予期しないエラーが発生しました\nPossible type: ${error}`, message.channel);
      }
  },
};
